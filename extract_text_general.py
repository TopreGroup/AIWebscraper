...
def extract_text_general():
    ...
    for url in urls:
        ...
        """ Write text to file """
        with open(file_name+str(file_num)+'.txt', 'w', encoding ='utf-8-sig') as f:
            f.write(text)
        
        """ Read file and store contents in set while preserving order """ 
        content = open(file_name+str(file_num)+'.txt', 'r', encoding='utf-8-sig')
        ...
        with open(file_name+str(file_num)+'.txt', 'w', encoding ='utf-8-sig') as fil:
            for line in texts:
        ...
        blob = bucket.blob("extracted_data/" + file_name+str(file_num)+'.txt')
        blob.upload_from_filename(file_name+str(file_num)+'.txt')
    ...
    filename = 'working_product_urls.txt'
    bucket = 'trunkedfilestorage' 
    body = {'name': 'working_product_urls.txt'}
